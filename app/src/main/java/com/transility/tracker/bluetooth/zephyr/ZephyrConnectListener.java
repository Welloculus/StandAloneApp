...
import com.transility.tracker.BuildConfig;
...
public class ZephyrConnectListener extends ConnectListenerImpl {
    ...
    public void Connected(ConnectedEvent<BTClient> eventArgs) {
        if (BuildConfig.DEBUG) {
            Log.v(Constants.TAG, String.format("Connected to BioHarness %s.", eventArgs.getSource().getDevice().getName()));
        }
        ...
        zephyrProtocol.addZephyrPacketEventListener(new ZephyrPacketListener() {
            public void ReceivedPacket(ZephyrPacketEvent eventArgs) {
                ...
                if (PACKET_HR_SPD_DIST == msg.getMsgID()) {
                    ...
                    b1.putInt(Constants.EXTRAS_HEART_RATE, HRate);
                    text1.setData(b1);
                    handler.sendMessage(text1);
                    if (BuildConfig.DEBUG) {
                        Log.v(Constants.TAG, "Heart Rate is " + HRate);
                    }
                    ...
                    handler.sendMessage(text1);
                    if (BuildConfig.DEBUG) {
                        Log.v(Constants.TAG, "Instant Speed is " + InstantSpeed);
                    }
                }
            }
        });
